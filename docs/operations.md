...
`keystone-algo-graph-flow` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for graph analysis with fixture event logs, golden state snapshots, and documented operating limits.
...
The score is deterministic. The current threshold is 165. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 4.
